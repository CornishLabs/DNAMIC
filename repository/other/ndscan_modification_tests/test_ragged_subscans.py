...
class LineExp(ExpFragment):
    ...
    def _analyse_grad(self, axis_values, result_values, analysis_results):
        ...
        popt, pcov = curve_fit(model, x, y)  # optionally: bounds=(0, np.inf)
        m = popt[0]
...
class ScanXExpFrag(SubscanExpFragment):
    ...
    def _configure(self):
        gen = LinearGenerator(0.0, 5.0, 6, True)
        opts = ScanOptions(
            num_repeats=1, num_repeats_per_point=1, randomise_order_globally=True
        )
...
class HowDoesPVaryExpFrag(SubscanExpFragment):
    ...
    def _configure(self):
        gen = LinearGenerator(0.0, 5.0, 10, True)
        opts = ScanOptions(
            num_repeats=1, num_repeats_per_point=1, randomise_order_globally=True
        )
...
HowDoesPVaryExp = make_fragment_scan_exp(HowDoesPVaryExpFrag)
